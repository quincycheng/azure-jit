...
@app.route('/azure/cleanup_sp' , methods=['POST'])
def cleanup_service_principal():
    ttl = os.popen('summon /app/cleanup.sh').read()
    return '{"result":"ok"}'
...
#    expire = datetime.datetime.now() + datetime.timedelta(minutes= ttl )
    ...
    azureCmd = os.popen('summon /app/create_sp.sh').read()
    ...
    azureCmd2 = os.popen('summon /app/assign_role.sh '+result_data["appId"] +' '+ azure_role ).read()
